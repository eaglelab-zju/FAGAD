...
class Dgldlog:
    ...
    def update_runs(self):
        sys.stdout = Logger(
            self.exp_path + "/" + self.exp_name + f"_run{self.run_num}" +
            ".log",
            self.terminal,
        )
        self.run_num += 1
        ...
        res = pd.DataFrame(
            {
                ...
                "structural anomaly score": s_list,
            },
            index=index,
        )
        res.to_markdown(self.exp_path + "/auc_res.md")

    def save_result(self, res_list_final, res_list_attrb, res_list_struct,
                    seed_list, args):
        result = {}
        ...
        with open(self.exp_path + "/" + self.exp_name + ".json",
                  "w") as json_file:
            json_file.write(json.dumps(result, ensure_ascii=False, indent=4))
        self.auc_result(res_list_final, res_list_attrb, res_list_struct,
                        seed_list)
