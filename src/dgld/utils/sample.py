...
class CoLASubGraphSampling(BaseSubGraphSampling):
    ...
    def __call__(self, g, start_nodes):
        ...
        paces = dgl.sampling.random_walk(g,
                                         start_nodes,
...
def generate_random_walk(g,
    ...
    if len(start_nodes) == 1:
        paces = [
            dgl.sampling.random_walk(g,
                                     start_nodes,
                                     length=length,
                                     restart_prob=prob_tensor)[0][:, 1:]
            for _ in range(multi_times)
        ]
    else:
        paces = [
            dgl.sampling.random_walk(g,
...
def generate_random_walk_singleThread(g,
                                      start_nodes,
                                      length,
                                      multi_length,
                                      restart_prob,
...
def generate_random_walk_multiThread(g, start_nodes, length, multi_length,
                                     restart_prob):
    """
...
def generate_random_walk_multiThread_high_level(g,
                                                start_nodes_block,
                                                paces_block,
                                                length,
                                                multi_length,
    ...
    for start, pace in zip(start_nodes_block, paces_block):
        ...
        while len(pace) < length - 1:
            ...
            if False:
                ...
                pace = [
                    dgl.sampling.random_walk(g, [start],
                                             length=length,
                                             restart_prob=prob_tensor)[0]
                    for _ in range(multi_times)
                ]
                ...
                # exit()
            # print('process_1')
            pace = generate_random_walk(g, [start],
                                        length=length,
                                        multi_length=5,
                                        restart_prob=0.9)
            pace = pace.tolist()[0]
...
class SLGAD_SubGraphSampling(BaseSubGraphSampling):
    ...
    def __call__(self, g, start_nodes, block_ID=None):
        ...
        # print(bar)
        start_nodes_block = [
            start_nodes[bar * i:bar * (i + 1)] for i in range(block)
        ]
        ...
        for i in range(block):
            temp_result = p.apply_async(
                generate_random_walk_multiThread_high_level,
                args=(
                    g,
                    start_nodes_block[i],
                    paces_block[i],
