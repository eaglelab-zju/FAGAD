...
def parse_all_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="DGLD",
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        "The number of runs of task with same parmeter,If the number of runs is not 1, \
    ...
    if "--dataset" in arg_list:
        ...
        dataset = [
            x.split("=")[-1] for x in arg_list if x.startswith("--dataset=")
        ]
        dataset = dataset[0]
    else:
    ...
    if dataset in IN_FEATURE_MAP.keys():
        parser.set_defaults(feat_dim=IN_FEATURE_MAP[dataset],
