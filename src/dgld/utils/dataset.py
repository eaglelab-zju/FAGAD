...
class GraphNodeAnomalyDectionDataset(DGLDataset):
    ...
    def __init__(
        ...
        if self.dataset_name != "custom" and self.dataset_name != "ACM":
            assert (self.dataset_name in self.q_map and self.dataset_name
                    in self.dataset_map), self.dataset_name
        ...
        if self.dataset_name != "custom" and y_data == None:
            print(
                "inject_contextual_anomalies and inject_structural_anomalies")
    ...
    def init_anomaly_label(self, label=None):
        ...
        if label != None:
            self.dataset.ndata[
                "anomaly_label"] = label  # torch.zeros(number_node)
        else:
    ...
    def inject_structural_anomalies(self):
        ...
        print("inject structural_anomalies numbers:",
              len(self.structural_anomalies_idx))
    ...
    def inject_contextual_anomalies(self):
        ...
        for aa_i in attribute_anomalies_idx:
            ...
            random_k_idx = np.random.choice(all_nodes_idx,
                                            size=k,
                                            replace=False)
        ...
        self.set_anomaly_label(labels)
        print("inject contextual_anomalies numbers:",
              len(self.contextual_anomalies_idx))
        print("anomalies numbers:", len(self.anomalies_idx))
...
def test_custom_dataset():
    ...
    dataset = GraphNodeAnomalyDectionDataset("custom",
                                             g_data=my_g,
                                             y_data=label)
