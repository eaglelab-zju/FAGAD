...
if __name__ == "__main__":
    ...
    data_path = "dgld/data"
    save_path = "dgld/models/SCAD/results/clu_pre_1123.csv"
    runs = 1
    ...
    n_clusters_list = [3, 5, 7, 10, 20]
    pre_epochs_list = [0, 10, 50, 100, 150]
    ...
    for data_name in Data_name_list:
        ...
        for n_cluster in n_clusters_list:
            print("-" * 20, f"n_cluster:{n_cluster}", "-" * 20)
            for pre_epochs in pre_epochs_list:
                print("-" * 20, f"pre_epochs:{pre_epochs}", "-" * 20)
                for run_id in range(runs):
                    ...
                    if data_name not in ["BlogCatalog", "Flickr"]:
                        ...
                        graph.ndata["feat"] = torch.from_numpy(
                            np.array(tmp_feat))
...
# CUDA_VISIBLE_DEVICES=4 nohup python -u -m dgld.models.SCAD.models_clu_pre > ../logs/SCAD/clu_pre_1123.log 2>&1 &
