...
pkl_path = "/data1/ygm/projects/DGLD_idea/pkls/emb_dict.pt"
af_pkl_path = "/data1/ygm/projects/DGLD_idea/pkls/af_emb_dict.pt"
    ...
    def fit(
        ...
        if batch_size == 0:
            ...
            for epoch in range(num_epochs):
                ...
                # # 保存训练前的low high emb
                # emb_dict = {
                #     'low_emb':low_emb.detach().cpu(),
                #     'high_emb':high_emb.detach().cpu(),
                #     'label':label,
                #     'cluster': Q.detach().data.cpu().numpy().argmax(1)
                # }
                # torch.save(emb_dict,pkl_path)
                # print(pkl_path)
                # exit()
                ...
                if epoch % 50 == 0:
                    ...
                    torch.save(emb_dict, af_pkl_path)
                    print(af_pkl_path)
                    exit()
...
if __name__ == "__main__":
    ...
    data_path = "dgld/data"
    save_path = None  #'dgld/models/SCAD/results/Flickr_1122.csv'
    runs = 1
    seed_list = [random.randint(0, 99999) for i in range(runs)]

    # Data_name_list=['BlogCatalog','Flickr','wiki','reddit','Amazon','Enron']
    Data_name_list = ["BlogCatalog"]
...
# CUDA_VISIBLE_DEVICES=0 nohup python -u -m dgld.models.SCAD.models_tsne > ../logs/SCAD/Flickr_1122.log 2>&1 &
# CUDA_VISIBLE_DEVICES=2 python -u -m dgld.models.SCAD.models_tsne
