...
class CoLADataSet(DGLDataset):
    ...
    def __init__(self, g, subgraphsize=4):
        ...
        self.dataset = g
        self.colasubgraphsampler = CoLASubGraphSampling(
    ...
    def random_walk_sampling(self):
        ...
        """
        self.paces = self.colasubgraphsampler(
            self.dataset, list(range(self.dataset.num_nodes())))
    ...
    def __getitem__(self, i):
        ...
        """
        pos_subgraph = self.graph_transform(
            dgl.node_subgraph(self.dataset, self.paces[i]))
        neg_idx = np.random.randint(self.dataset.num_nodes())
        while neg_idx == i:
            neg_idx = np.random.randint(self.dataset.num_nodes())
        neg_subgraph = self.graph_transform(
            dgl.node_subgraph(self.dataset, self.paces[neg_idx]))
