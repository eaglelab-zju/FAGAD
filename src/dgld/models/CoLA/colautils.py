...
def train_epoch(epoch, loader, net, device, criterion, optimizer):
    ...
    for step, (pos_subgraph,
        ...
        optimizer.zero_grad()
        pos_scores, neg_scores = net(pos_subgraph, posfeat, neg_subgraph,
                                     negfeat)
    ...
    print(
        "TRAIN==>epoch",
        epoch,
        "Average training loss: {:.2f}".format(loss_accum),
        # color="blue",
    )
    return loss_accum
...
def test_epoch(epoch, loader, net, device, criterion):
    ...
    for step, (pos_subgraph,
        ...
        predict_scores.extend(
            list((torch.sigmoid(neg_scores) -
                  torch.sigmoid(pos_scores)).detach().cpu().numpy()))
        loss = loss_fun(pos_scores, neg_scores, criterion, device)
        loss_accum += loss.item()
    loss_accum /= step + 1
    print(
        "VALID==>epoch",
        epoch,
        "Average valid loss: {:.2f}".format(loss_accum),
        # color="blue",
    )
