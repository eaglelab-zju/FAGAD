...
class OneLayerGCN(nn.Module):
    ...
    def forward(self, bg, in_feat):
        ...
        return (
            ...
            F.normalize(anchor_out, p=2, dim=1),
        )
...
class SL_GAD_Model(nn.Module):
    ...
    def forward(self, pos_batchg, pos_in_feat, neg_batchg, neg_in_feat, args):
        ...
        score_tot = torch.cat(
            (
                torch.cat((pos_scores_1, pos_scores_2), dim=1),
                torch.cat((neg_scores_1, neg_scores_2), dim=1),
            ),
            dim=0,
        )
        ...
        return (
            ...
            contrastive_loss,
            generative_loss,
        )  # , contrastive_score, generative_score
...
class SLGAD:
    ...
    def fit(
        ...
        g_p = Path(f"./tmp/SLGAD/{g.name}.{batch_size}")
        if g_p.exists():
            train_loader = torch.load(g_p)
        else:
            make_parent_dirs(g_p)
            dataset = SL_GAD_DataSet(base_dataset_name="custom", g_data=g)
            train_loader = GraphDataLoader(
                dataset,
                batch_size=batch_size,
            ...
            torch.save(train_loader, g_p, pickle_protocol=4)
        print(train_loader)
        # exit()
    ...
    def predict(self,
        ...
        if g_p.exists():
            ...
            torch.save(test_loader, g_p, pickle_protocol=4)
