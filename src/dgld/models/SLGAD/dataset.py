...
class SL_GAD_DataSet(DGLDataset):
    ...
    def __init__(
        ...
        self.oraldataset = GraphNodeAnomalyDectionDataset(
            name=self.dataset_name, g_data=g_data, y_data=y_data)
        self.dataset = self.oraldataset[0]
        # print(self.dataset)
        # exit()
        # print(self.dataset.ndata['feat'][:5, :5])
        # exit()
        self.SLGAD_subgraphsampler = SLGAD_SubGraphSampling(
            length=self.subgraphsize)
        self.paces = []
        # self.write_graph()
        self.normalize_feat()
        self.normalize_adj()
    ...
    def adj_matrix(self):
        ...
        # print(src)
        # print(dst)
        adj_matrix = sp.coo_matrix((edge_weight, (src, dst)),
    ...
    def normalize_feat(self):
        ...
        # print(self.dataset.ndata['feat'][:5, :5])
        # exit()
        self.dataset.ndata["feat"] = F.normalize(self.dataset.ndata["feat"],
                                                 p=1,
                                                 dim=1)
    ...
    def normalize_adj(self):
        ...
        norm_edge_weight = torch.cat((norm_edge_weight, self_loop_weight),
                                     dim=0)
    ...
    def random_walk_sampling(self):
        ...
        self.paces_1 = self.SLGAD_subgraphsampler(
            self.sample_graph, list(range(self.sample_graph.num_nodes())))
        time_1 = datetime.now()
        self.paces_2 = self.SLGAD_subgraphsampler(
            self.sample_graph, list(range(self.sample_graph.num_nodes())))
        time_2 = datetime.now()
        self.paces_3 = self.SLGAD_subgraphsampler(
            self.sample_graph, list(range(self.sample_graph.num_nodes())))
        time_3 = datetime.now()
