...
def set_subargs(parser):
    ...
    parser.add_argument("--global_adg",
    ...
    parser.add_argument("--attention",
                        type=lambda x: x.lower() == "true",
                        default=False)
    parser.add_argument("--positive_subgraph_cor",
                        type=lambda x: x.lower() == "true",
                        default=False)
    parser.add_argument("--negative_subgraph_cor",
                        type=lambda x: x.lower() == "true",
                        default=False)
    parser.add_argument("--arw",
                        type=lambda x: x.lower() == "true",
                        default=False)
...
def train_epoch(epoch, args, loader, net, device, criterion, optimizer):
    ...
    for step, (pos_subgraph_1, pos_subgraph_2, neg_subgraph,
               idx) in enumerate(tqdm(loader, desc="Iteration")):
        end_time = datetime.now()
        ...
        posfeat = [posfeat_1, posfeat_2, raw_posfeat_1,
                   raw_posfeat_2]  # .to(device)
        negfeat = neg_subgraph.ndata["feat"].to(device)
        optimizer.zero_grad()

        loss, single_predict_scores, single_contrastive_loss, single_generative_loss = (
            net(pos_subgraph, posfeat, neg_subgraph, negfeat, args))
        loss.backward()
        optimizer.step()
        loss_accum += loss.cpu().detach().numpy(
        ) * pos_subgraph_1.number_of_nodes()
        contrastive_loss_accum += (
            single_contrastive_loss.cpu().detach().numpy() *
            pos_subgraph_1.number_of_nodes())
        generative_loss_accum += (
            single_generative_loss.cpu().detach().numpy() *
            pos_subgraph_1.number_of_nodes())
    ...
    print(
        "TRAIN==>epoch",
        epoch,
        "Average training loss: {:.8f}".format(loss_accum),
        # color="blue",
    )
...
def test_epoch(epoch, args, loader, net, device, criterion, optimizer):
    ...
    net.eval()
    number_nodes = 0
    predict_scores = torch.full(
        [
            loader.dataset.dataset.number_of_nodes(),
        ],
        0.0,
    )
    ...
    for step, (pos_subgraph_1, pos_subgraph_2, neg_subgraph,
        ...
        contrastive_loss_accum += (single_contrastive_loss.item() *
                                   pos_subgraph_1.number_of_nodes())
        generative_loss_accum += (single_generative_loss.item() *
                                  pos_subgraph_1.number_of_nodes())
    ...
    print(
        "VALID==>epoch",
        epoch,
        "Average valid loss: {:.8f}".format(loss_accum),
        # color="blue",
    )
    print("contrastive loss : ", contrastive_loss_accum)
    print("generative loss : ", generative_loss_accum)
