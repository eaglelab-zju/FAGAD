...
class PolyConvBatch(nn.Module):
    ...
    def forward(self, block, feat):
        ...
        with block.local_scope():
            D_invsqrt = (torch.pow(block.out_degrees().float().clamp(min=1),
                                   -0.5).unsqueeze(-1).to(feat.device))
            h = self._theta[0] * feat
            for k in range(1, self._k):
...
class BernNet(nn.Module):
    """https://github.com/ivam-he/BernNet/blob/main/NodeClassification/models.py"""
    ...
    def forward(self, g, in_feat):
        in_feat = F.dropout(in_feat, p=0.5, training=self.training)
        ...
        h = F.dropout(h, p=0.5, training=self.training)
        # h = self.act(h)
        h_final = torch.zeros([len(in_feat), self.h_feats]).to(in_feat.device)
...
# heterogeneous graph
class BernNet_Hetero(nn.Module):

    def __init__(self, in_feats, h_feats, num_classes, graph, d=2):
        super(BernNet_Hetero, self).__init__()
        self.g = graph
        self.thetas = calculate_theta2(d=d)
        self.h_feats = h_feats
        self.conv = [
