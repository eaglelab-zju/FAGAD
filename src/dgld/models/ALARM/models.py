...
from .alarm_utils import normalize_adj
from .alarm_utils import test_step
from .alarm_utils import train_step
...
class Encoder(nn.Module):
    ...
    def __init__(self, nfeat, nhid, dropout, view_num, agg_type, agg_vec):
        ...
        self.gc1 = nn.ModuleList(
            GraphConv(self.view_feat[i], nhid, norm="none")
...
class ALARMModel(nn.Module):
    ...
    def __init__(self, feat_size, hidden_size, dropout, view_num, agg_type,
                 agg_vec):
        super(ALARMModel, self).__init__()
        if len(agg_vec) != view_num:
            raise KeyError(
                "Aggregator vector size is {}, but the number of view is {}".
                format(len(agg_vec), view_num))
        self.view_num = view_num
        self.shared_encoder = Encoder(feat_size, hidden_size, dropout,
                                      view_num, agg_type, agg_vec)
...
class ALARM(nn.Module):
    ...
    def __init__(self, feat_size, hidden_size, dropout, view_num, agg_type,
        ...
        self.model = ALARMModel(feat_size, hidden_size, dropout, view_num,
                                agg_type, agg_vec)
    ...
    def fit(self, graph, lr=5e-3, num_epoch=1, alpha=0.8, device="cpu"):
        ...
        for epoch in range(num_epoch):
            ...
            loss, struct_loss, feat_loss = train_step(self.model, optimizer,
                                                      graph, features,
                                                      adj_label, alpha)
            print(
                "Epoch:",
    ...
    def predict(self, graph, alpha=0.8, device="cpu"):
        ...
        predict_score = test_step(self.model, graph, features, adj_label,
                                  alpha)
