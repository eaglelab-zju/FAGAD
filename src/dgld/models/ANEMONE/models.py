...
from .anemone_utils import test_epoch
from .anemone_utils import train_epoch
...
class OneLayerGCNWithGlobalAdg(nn.Module):
    ...
    def forward(self, bg, in_feat):
        ...
        return (
            F.normalize(subgraph_pool_emb, p=2, dim=1),
            F.normalize(subgraph_rec_emb, p=2, dim=1),
            F.normalize(anchor_out_1, p=2, dim=1),
            F.normalize(anchor_out_2, p=2, dim=1),
        )
...
class AneModel(nn.Module):
    ...
    def forward(self, pos_batchg, pos_in_feat, neg_batchg, neg_in_feat):
        ...
        neg_scores_rec : Torch.tensor
            anomaly score of negative sample
        """
        pos_pool_emb, pos_rec_emb, anchor_out_pos_1, anchor_out_pos_2 = self.gcn(
            pos_batchg, pos_in_feat)
        neg_pool_emb, neg_rec_emb, _, _ = self.gcn(neg_batchg, neg_in_feat)
        pos_scores_rdt, pos_scores_rec = self.discriminator(
            pos_pool_emb, pos_rec_emb, anchor_out_pos_1, anchor_out_pos_2)
        ...
        neg_scores_rdt, neg_scores_rec = self.discriminator(
            neg_pool_emb, neg_rec_emb, anchor_out_pos_1, anchor_out_pos_2)
        return (
            pos_scores_rdt[:, 0],
            pos_scores_rec[:, 0],
            neg_scores_rdt[:, 0],
...
class ANEMONE:
    ...
    def predict(
        ...
        for rnd in range(auc_test_rounds):
            ...
            predict_score = test_epoch(rnd, alpha, test_loader, self.model,
                                       device, self.criterion)
