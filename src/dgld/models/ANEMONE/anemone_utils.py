...
def loss_fun_BCE(
    ...
    return (alpha) * torch.mean(loss_rdc) + (
        1 - alpha) * torch.mean(loss_rec), accuarcy
...
def train_epoch(epoch, alpha, loader, net, device, criterion, optimizer):
    ...
    loss_accum = 0
    net.train()
    for step, (pos_subgraph,
               neg_subgraph) in enumerate(tqdm(loader, desc="Iteration")):
        pos_subgraph, neg_subgraph = pos_subgraph.to(device), neg_subgraph.to(
            device)
        posfeat = pos_subgraph.ndata["feat"].to(device)
        negfeat = neg_subgraph.ndata["feat"].to(device)
        optimizer.zero_grad()
        pos_scores_rdt, pos_scores_rec, neg_scores_rdt, neg_scores_rec = net(
...
def test_epoch(epoch, alpha, loader, net, device, criterion):
    ...
    for step, (pos_subgraph,
        ...
        predict_scores.extend(
            list(alpha *
                 (torch.sigmoid(neg_scores_rdt) -
                  torch.sigmoid(pos_scores_rdt)).detach().cpu().numpy() +
                 (1 - alpha) *
                 (torch.sigmoid(neg_scores_rec) -
                  torch.sigmoid(pos_scores_rec)).detach().cpu().numpy()))
        loss, acc = loss_fun(
            pos_scores_rdt,
            pos_scores_rec,
            neg_scores_rdt,
