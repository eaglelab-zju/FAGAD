...
from .functional import KL_neighbor_loss
from .functional import W2_neighbor_loss
from .nn import FNN_GAD_NR
from .nn import MLP_generator
...
class GADNRBase(nn.Module):
    ...
    **kwargs : optional
    ...
    def __init__(self,
        ...
        if self.full_batch:  # full batch mode
            ...
            self.m_fullbatch = torch.distributions.Normal(
                torch.zeros(sample_size, self.tot_node, hid_dim),
                torch.ones(sample_size, self.tot_node, hid_dim),
            )
            self.mean_agg = SAGEConv(hid_dim,
                                     hid_dim,
                                     aggr="mean",
                                     normalize=False)
            ...
            self.m_minibatch = torch.distributions.Normal(
                torch.zeros(sample_size, hid_dim),
        ...
        self.feature_decoder = FNN_GAD_NR(hid_dim, hid_dim, hid_dim,
                                          fea_dec_layers)
    ...
    def sample_neighbors(self, input_id, neighbor_dict, id_mapping,
                         gt_embeddings):
        ...
        for index in input_id:
            ...
            for index in sample_indexes:
                sampled_embeddings.append(
                    gt_embeddings[id_mapping[index]].tolist())
    ...
    def mini_batch_neigh_recon(self, h1, h0, input_id, neighbor_dict,
        ...
        sampled_embeddings_list, mask_len_list = self.sample_neighbors(
            input_id, neighbor_dict, id_mapping, h0)
        for index, neighbor_embeddings in enumerate(sampled_embeddings_list):
            ...
            target_neighbors = torch.unsqueeze(
                torch.FloatTensor(neighbor_embeddings), dim=0).to(self.device)
    ...
    def forward(self,
        ...
        for _ in range(self.sample_time):
            ...
            if self.full_batch:  # full batch mode
                neigh_recon_info = self.full_batch_neigh_recon(
                    h1, h0, edge_index)
            else:  # mini batch mode
                neigh_recon_info = self.mini_batch_neigh_recon(
                    h1, h0, input_id, neighbor_dict, id_mapping)
    ...
    def loss_func(
        ...
        for t in range(self.sample_time):
            ...
            if self.full_batch:
                ...
                for generated_neighbors, target_neighbors, mask_len in zip(
                        gen_neighs, tar_neighs, mask_lens):
                    temp_loss = self.neighbor_loss(generated_neighbors,
                                                   target_neighbors, mask_len,
                                                   self.device)
                    local_index_loss += temp_loss
                    local_index_loss_per_node.append(temp_loss)

                local_index_loss_per_node = torch.stack(
                    local_index_loss_per_node)
