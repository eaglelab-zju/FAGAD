...
class CARDBase(nn.Module):
    ...
    **kwargs
    ...
    def forward(self, data):
        ...
        perm_idx = torch.randperm(self.disc_emb.shape[0]).to(
            self.disc_emb.device)
    ...
    def loss_func(self, logits, diff_logits, x_, local_x_, x, con_label):
        ...
        constra_loss = (torch.mean(logit_loss) + inter_logit_loss +
                        self.gama * torch.mean(local_rec_loss))

        final_loss = (1 - self.fp) * constra_loss + self.fp * torch.mean(
            rec_loss)  # + 0.5 * kl

        constra_score = (
            (logits[batch_size:] - logits[:batch_size]) +
            (diff_logits[batch_size:] - diff_logits[:batch_size])) / 2

        score = (1 - self.fp) * (constra_score +
                                 self.gama * local_rec_loss[:batch_size]
                                 ) + self.fp * rec_loss[:batch_size]
    ...
    def _train_subgraph_network(self, data):
        ...
        for index in range(data.num_nodes):
            subgraphs = NeighborLoader(
                data,
                num_neighbors=[self.subgraph_num_neigh] * self.num_layers)
