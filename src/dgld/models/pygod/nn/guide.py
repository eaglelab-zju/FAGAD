    ...
    **kwargs
    ...
    def calc_gdd(data, cache_dir=None, graphlet_size=4, selected_motif=True):
        ...
        file_name = ("nmd_" + str(hash_func.hexdigest()[:8]) +
                     str(graphlet_size) + str(selected_motif)[0] + ".pt")
        ...
        if os.path.exists(file_path):
            ...
            features = {
                node: {
                    i: 0
                    for i in range(unique_motif_count)
                }
                for node in g.nodes()
            }
            ...
            motifs = [[n] +
                      [features[n][i] for i in range(unique_motif_count)]
                      for n in g.nodes()]
