        ...
        if self.warmup > 0:
            ...
            if not self.clustered:
                ...
                kmeans = KMeans(n_clusters=self.k,
                                n_init="auto").fit(emb.detach().cpu())
                self.cluster = torch.tensor(kmeans.labels_).long().to(
                    emb.device)
                self.centers = torch.Tensor(kmeans.cluster_centers_).to(
                    emb.device)
            else:
            ...
            score = torch.relu(
                torch.sum(torch.pow(emb - self.centers[self.cluster], 2), 1) -
