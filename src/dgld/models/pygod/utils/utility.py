...
def check_parameter(
    ...
    if (include_left and include_right) and (param < low or param > high):
        raise ValueError("{param_name} is set to {param}. "
    ...
    elif (include_left and not include_right) and (param < low
                                                   or param >= high):
        raise ValueError("{param_name} is set to {param}. "
                         "Not in the range of [{low}, {high}).".format(
                             param=param,
    ...
    elif (not include_left and include_right) and (param <= low
                                                   or param > high):
        raise ValueError("{param_name} is set to {param}. "
                         "Not in the range of ({low}, {high}].".format(
                             param=param,
    ...
    elif (not include_left and not include_right) and (param <= low
                                                       or param >= high):
        raise ValueError("{param_name} is set to {param}. "
                         "Not in the range of ({low}, {high}).".format(
                             param=param,
    ...
    if verbose > 0:
        if deep:
            ...
            if isinstance(loss, tuple):
                print("Loss I {:.4f} | Loss O {:.4f} | ".format(
                    loss[0], loss[1]),
                      end="")
            else:
        ...
        if verbose > 1:
            if target is not None:
                ...
                auc, a_score, s_score = split_auc(target, score)
            ...
            if verbose > 2:
                if target is not None:
                    ...
                    print(
                        ...
                        "| AP {:.4f} | F1 {:.4f}".format(rec, pre, ap, f1),
                        end="",
                    )
...
def is_fitted(detector, attributes=None):
    ...
    if attributes is None:
        attributes = ["model"]
    assert all(
        hasattr(detector, attr) and eval("detector.%s" % attr) is not None
        for attr in attributes), "The detector is not fitted yet"
