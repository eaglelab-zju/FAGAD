...
class GADNR(DeepDetector):
    ...
    **kwargs : optional
    ...
    def process_graph(self, data):
        if not self.full_batch or self.batch_size != data.x.shape[
                0]:  # mini-batch
            data, neighbor_dict, neighbor_num_list, id_mapping = (
                GADNRBase.process_graph(data, data.input_id.tolist()))
        else:  # full batch
            data, neighbor_dict, neighbor_num_list, id_mapping = (
    ...
    def forward_model(self, data):
        if not self.full_batch:  # mini-batch training
            h0, degree_logits, feat_recon_list, neigh_recon_list = self.model(
                data.x,
                data.edge_index,
        ...
        loss, loss_per_node, h_loss, degree_loss, feature_loss = self.model.loss_func(
    ...
    def fit(
        ...
        optimizer = torch.optim.Adam(
            [
                ...
                {
                    "params": self.model.degree_decoder.parameters(),
                    "lr": 1e-2
                },
            ],
            lr=self.lr,
        ...
        for epoch in range(1, self.epoch + 1, 1):
            ...
            if self.full_batch:
                ...
                comp_loss = self.comp_decision_score(
                    loss_per_node,
                    h_loss,
                    degree_loss,
                    feature_loss,
                ...
                for sampled_data in loader:
                    ...
                    self.decision_score_[
                        node_idx[:batch_size]] = comp_loss.squeeze(1)
                    ...
                    epoch_loss_per_node[
                        node_idx[:batch_size]] = loss_per_node.squeeze(1)
    ...
    def decision_function(
        ...
        if self.batch_size == data.x.shape[0]:  # full batch inference
            loss, loss_per_node, h_loss, degree_loss, feature_loss = self.forward_model(
                data)
            comp_loss = self.comp_decision_score(
                loss_per_node,
                h_loss,
