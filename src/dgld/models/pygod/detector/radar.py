...
class Radar(Detector):
    ...
    def process_graph(self, data):
        ...
        r_init = (torch.inverse(
            (1 + self.weight_decay) * torch.eye(x.shape[0]).to(self.device) +
            self.gamma * laplacian) @ x)

        return x, s, laplacian, w_init, r_init

    def _loss(self, x, x_, r, l):
        return torch.norm(x - x_ - r,
                          2) + self.gamma * torch.trace(r.T @ l @ r)
