...
class AAGNN_batch(nn.Module):
    ...
    def predict(self, graph, device="cpu"):
        ...
        with torch.no_grad():
            h = self.model(graph, features)
            score = torch.sum(torch.pow(h - self.center, 2),
                              dim=-1).detach().cpu()
    ...
    def cal_center(self, graph, x, rate):
        ...
        with torch.no_grad():
            ...
            dis = torch.sum(torch.pow(center.repeat(h.shape[0], 1) - h, 2),
                            dim=-1)
...
class detGAT(nn.Module):
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            ...
            graph.edata["w"] = dgl.nn.functional.edge_softmax(
                graph, graph.edata["cat"])
