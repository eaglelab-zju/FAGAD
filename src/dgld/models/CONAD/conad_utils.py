...
def train_step_batch(model, optimizer, criterion, g_orig, g_aug, alpha, eta,
                     batch_size, device):
    """
    ...
    for (input_nodes_orig, output_nodes_orig, blocks_orig), (
        ...
        adj_orig = (dgl.block_to_graph(blocks_orig[-1]).adj().to_dense().to(
            label.device)[:output_nodes_num, :output_nodes_num])
        # print(h_orig.shape, h_aug.shape, adj_orig.shape, label.shape)
        contrast_loss = criterion(h_orig[:output_nodes_num],
                                  h_aug[:output_nodes_num], label, adj_orig)
        # recontruct loss
...
def test_step(model, graph, alpha):
    ...
    adj_orig = graph.adj_external().to_dense().cpu()
