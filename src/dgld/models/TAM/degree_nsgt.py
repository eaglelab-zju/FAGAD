...
def calc_dis(adj_matrix, attr_matrix):
    ...
    for i in range(row):
        ...
        for j in node_index:
            dis = np.sqrt(
                np.sum((attr_matrix[i] - attr_matrix[j]) *
                       (attr_matrix[i] - attr_matrix[j])))
            dis_array[i][j] = dis
            if dis > max_dis:
                max_dis = dis_array[i][j]
            if dis < min_dis:
    ...
    for i in range(row):
        ...
        if node_index.shape[0] != 0:
            ...
            if max_dis > min_dis:
                ...
                cutting_edge = np.argwhere(
                    dis_array[i, node_index] > random_value)
...
for i in range(N_t):
    ...
    score1 = (np.sum(origin_adj, 0) - np.sum(new_adj_matrix, 0)) / (np.sum(
        origin_adj, 0))
    adj_matrix = new_adj_matrix
    score1[np.isinf(score1)] = 0.0
    score1[np.isnan(score1)] = 0.0
    score = score1
