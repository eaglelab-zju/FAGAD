...
def normalize_score(ano_score):
    ano_score = (ano_score - np.min(ano_score)) / (np.max(ano_score) -
                                                   np.min(ano_score))
    return ano_score
...
def calc_sim(adj_matrix, attr_matrix):
    ...
    for i in range(row):
        ...
        for j in node_index:
            dis = get_cos_similar(attr_matrix[i].tolist(),
                                  attr_matrix[j].tolist())
            dis_array[i][j] = dis
...
def graph_nsgt(dis_array, adj):
    ...
    for i in range(row):
        ...
        if node_index.shape[0] != 0:
            ...
            min_dis = mean_dis
            if max_dis > min_dis:
                random_value = (max_dis -
                                min_dis) * np.random.random_sample() + min_dis
                cutting_edge = torch.argwhere(
                    dis_array[i, node_index[:, 0]] > random_value)
...
def draw_pdf(message, ano_label, dataset):
    with PdfPages("{}-TAM.pdf".format(dataset)) as pdf:
        ...
        n, bins, patches = plt.hist(message_all,
                                    bins=30,
                                    normed=1,
...
def draw_pdf_str_attr(message, ano_label, str_ano_label, attr_ano_label,
                      dataset):
    with PdfPages("{}-TAM.pdf".format(dataset)) as pdf:
        normal_message_all = message[ano_label == 0]
        ...
        message_all = [
            normal_message_all,
            str_abnormal_message_all,
            attr_abnormal_message_all,
        ]
