...
class GAAN:
    ...
    def fit(
        ...
        for epoch in range(num_epoch):
            ...
            for input_nodes, output_nodes, blocks in dataloader:
                ...
                # train discriminator
                random_noise = torch.randn(x.shape[0],
                                           self.noise_dim).to(device)
                x_, a, a_ = self.model(random_noise, x)
                loss_d = self.dis_loss(a[src_node, dst_node],
                                       a_[src_node, dst_node].detach())
                opt_d.zero_grad()
                loss_d.backward()
                nn.utils.clip_grad_norm_(self.model.discriminator.parameters(),
                                         0.001)
                opt_d.step()
                ...
                if verbose and y_true is not None:
                    ...
                    score[output_nodes] = (self.cal_score(
                        subgraph, x, x_, a, alpha,
                        len(output_nodes)).detach().cpu())
            ...
            if verbose:
                print(
                    ...
                    "%04d" % (epoch),
                    "generator_loss=",
                    "{:.5f}".format(epoch_loss_g),
                    "discriminator_loss=",
                    "{:.5f}".format(epoch_loss_d),
                )
    ...
    def predict(
        ...
        for input_nodes, output_nodes, blocks in dataloader:
            ...
            subgraph = graph.subgraph(input_nodes, relabel_nodes=True)
            score[output_nodes] = (self.cal_score(
                subgraph, x, x_, a, alpha, len(output_nodes)).detach().cpu())
        return score
    ...
    def cal_score(self, graph, x, x_, a, alpha, batch_size):
        ...
        adj = graph.adj_external().to_dense()[:batch_size].to(a.device)
        a = a[:batch_size]
    ...
    def gen_loss(self, a_):
        ...
        torch.tensor
            Generator loss.
        """
        loss_g = F.binary_cross_entropy(a_,
                                        torch.ones_like(a_),
                                        reduction="mean")
        return loss_g
    ...
    def dis_loss(self, a, a_):
        ...
        torch.tensor
            Discriminator loss.
        """
        loss_d = F.binary_cross_entropy(a,
                                        torch.ones_like(a),
                                        reduction="mean")
        loss_d_ = F.binary_cross_entropy(a_,
                                         torch.zeros_like(a_),
                                         reduction="mean")
...
class GAAN_model(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.generator = MLP(noise_dim,
                             gen_hid_dims,
                             attrb_dim,
                             dropout=dropout,
                             act=act)
        self.discriminator = MLP(attrb_dim,
                                 ed_hid_dims,
                                 out_dim,
                                 dropout=dropout,
                                 act=act,
