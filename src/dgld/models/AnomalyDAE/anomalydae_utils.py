...
def train_step(model, optimizer, graph, features, adj_label, alpha, eta,
               theta):
    model.train()
    optimizer.zero_grad()
    A_hat, X_hat = model(graph, features)
    loss, struct_loss, feat_loss = loss_func(adj_label, A_hat, features, X_hat,
                                             alpha, eta, theta)
    l = torch.mean(loss)
    l.backward()
    optimizer.step()
...
def test_step(model, graph, features, adj_label, alpha, eta, theta):
    model.eval()
    A_hat, X_hat = model(graph, features)
    loss, _, _ = loss_func(adj_label, A_hat, features, X_hat, alpha, eta,
                           theta)
