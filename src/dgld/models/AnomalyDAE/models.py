...
from .anomalydae_utils import test_step
from .anomalydae_utils import train_step
...
class AnomalyDAE(nn.Module):
    ...
    def __init__(self, feat_size, num_nodes, embed_dim, out_dim, dropout):
        ...
        self.model = AnomalyDAEModel(
            in_feat_dim=feat_size,
            in_num_dim=num_nodes,
            embed_dim=embed_dim,
            out_dim=out_dim,
            dropout=dropout,
        )
    ...
    def fit(
        ...
        for epoch in range(num_epoch):
            loss, struct_loss, feat_loss, _ = train_step(
                self.model, optimizer, graph, features, adj_label, alpha, eta,
                theta)
            print(
                "Epoch:",
                "%04d" % (epoch),
...
class AnomalyDAEModel(nn.Module):
    ...
    def __init__(
        ...
        self.structure_AE = StructureAE(in_feat_dim, in_num_dim, embed_dim,
                                        out_dim, dropout)
        self.attribute_AE = AttributeAE(in_num_dim, embed_dim, out_dim,
                                        dropout)
...
class StructureAE(nn.Module):
    ...
    def __init__(self, in_dim, in_num_dim, embed_dim, out_dim, dropout):
        ...
        self.attention_layer = NodeAttention(
            embed_dim,
            nb_nodes=in_num_dim,
            # act=tf.nn.relu,
            act=lambda x: x,
