...
class GUIDE:
    ...
    def fit(
        ...
        if alpha is None:
            alpha = torch.std(struct_feat).detach() / (
                torch.std(struct_feat).detach() +
                torch.std(attrb_feat).detach())
        if batch_size == 0:
            # all data
        ...
        for epoch in range(num_epoch):
            ...
            for input_nodes, output_nodes, blocks in dataloader:
                ...
                attrb_rb, struct_rb = self.model(blocks, attrb_feat_simple,
                                                 struct_feat_simple)
                loss = self.model.cal_loss(attrb_feat_out, attrb_rb,
                                           struct_feat_out, struct_rb, alpha)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                if verbose:
                    if y_true is not None:
                        score[output_nodes] = self.model.get_all_score(
                            attrb_feat_out.detach(),
                            attrb_rb.detach(),
                            struct_feat_out.detach(),
                            struct_rb.detach(),
                            alpha,
                        ).cpu()
            if verbose:
...
class GNAConv(nn.Module):
    ...
    def forward(self, block, feat):
        ...
        with block.local_scope():
            ...
            block.edata["w"] = dglnn.functional.edge_softmax(
                block, block.edata["w_a"])
