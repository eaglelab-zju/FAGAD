...
def set_subargs(parser):
    parser.add_argument(
        ...
        default=64,
        help=
        "dimension of hidden embedding for attribute encoder (default: 64)",
    )
    parser.add_argument(
        "--struct_hid",
        type=int,
    ...
    parser.add_argument("--struct_dim",
                        type=int,
                        default=6,
    ...
    parser.add_argument("--batch_size",
                        type=int,
                        default=0,
                        help="batch_size, 0 for all data ")
...
def cal_motifs(nx_g, x, idx, sample=50):
    ...
    if idx == 3:
        ...
        for i in range(size):
            ...
            if len(adj_y) > sample:
                adj_y = set(
                    np.random.choice(list(adj_y), size=sample, replace=False))
            rx = set(adj_x[i + 1:])
            z_set = rx - adj_y
            for z in z_set:
                adj_z = set(nx_g[z].keys())
...
def get_struct_feat(graph: dgl.DGLGraph):
    ...
    path = Path(f"./tmp/{graph.name}.struc.emb")
    if path.exists():
    ...
    torch.save(struct_feat, f=path, pickle_protocol=4)
    return struct_feat
