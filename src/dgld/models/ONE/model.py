...
class ONE:
    ...
    def fit(self, graph, num_epoch=5):
        ...
        A = graph.adj_external().to_dense().numpy()
        ...
        temp1 = np.multiply(np.log(np.reciprocal(outl1 + eps)),
                            np.sum(temp1, axis=1))
        temp1 = np.sum(temp1)

        temp2 = C - np.matmul(U, V)
        temp2 = np.multiply(temp2, temp2)
        temp2 = np.multiply(np.log(np.reciprocal(outl2 + eps)),
                            np.sum(temp2, axis=1))
        temp2 = np.sum(temp2)

        temp3 = G.T - np.matmul(W, U.T)
        temp3 = np.multiply(temp3, temp3)
        temp3 = np.multiply(np.log(np.reciprocal(outl3 + eps)),
        ...
        for epoch in range(num_epoch):
            ...
            for i in range(G.shape[0]):
                for k in range(G.shape[1]):
                    Gik_numer = alpha * np.log(
                        np.reciprocal(outl1[i] + eps)
                    ) * np.dot(
                        H[k, :],
                        (A[i, :] -
                         (np.matmul(G[i], H) - np.multiply(G[i, k], H[k, :]))),
                    ) + gamma * np.log(np.reciprocal(outl3[i] + eps)) * np.dot(
                        U[i], W[k, :])
                    Gik_denom = alpha * np.log(
                        np.reciprocal(outl1[i] + eps)) * np.dot(
                            H[k, :], H[k, :]) + gamma * np.log(
                                np.reciprocal(outl3[i] + eps))
                    G[i, k] = Gik_numer / Gik_denom

            # The updation rule for H[k,j]
            for k in range(H.shape[0]):
                for j in range(H.shape[1]):
                    Hkj_numer = alpha * np.dot(
                        np.multiply(np.log(np.reciprocal(outl1 + eps)), G[:,
                                                                          k]),
                        (A[:, j] - (np.matmul(G, H[:, j]) -
                                    np.multiply(G[:, k], H[k, j]))),
                    )
                    Hkj_denom = alpha * (np.dot(
                        np.log(np.reciprocal(outl1 + eps)),
                        np.multiply(G[:, k], G[:, k]),
                    ))
            ...
            for i in range(U.shape[0]):
                for k in range(U.shape[1]):
                    Uik_numer_1 = (beta *
                                   np.log(np.reciprocal(outl2[i] + eps)) *
                                   (np.dot(
                                       V[k, :],
                                       (C[i] -
                                        (np.matmul(U[i, :], V) -
                                         np.multiply(U[i, k], V[k, :]))),
                                   )))
                    Uik_numer_2 = (gamma *
                                   np.log(np.reciprocal(outl3[i] + eps)) *
                                   np.dot(
                                       (G[i, :] -
                                        (np.matmul(U[i, :], W) -
                                         np.multiply(U[i, k], W[:, k]))),
                                       W[:, k],
                                   ))
                    Uik_denom = beta * np.log(
                        np.reciprocal(outl2[i] + eps)) * np.dot(
                            V[k, :], V[k, :]) + gamma * np.log(
                                np.reciprocal(outl3[i] + eps)) * np.dot(
                                    W[:, k], W[:, k])
            ...
            for k in range(V.shape[0]):
                for d in range(V.shape[1]):
                    Vkd_numer = beta * np.dot(
                        np.multiply(np.log(np.reciprocal(outl2 + eps)), U[:,
                                                                          k]),
                        (C[:, d] - (np.matmul(U, V[:, d]) -
                                    np.multiply(U[:, k], V[k, d]))),
                    )
                    Vkd_denom = beta * (np.dot(
                        np.log(np.reciprocal(outl2 + eps)),
                        np.multiply(U[:, k], U[:, k]),
                    ))
            ...
            outl3_numer = (gamma * (np.multiply(
                (G.T - WUTrans), (G.T - WUTrans))).sum(axis=0).T)
            outl3_denom = gamma * pow(np.linalg.norm(
                (G.T - WUTrans), "fro"), 2)
            outl3_numer = outl3_numer * mu
            outl3 = outl3_numer / outl3_denom

            loss = loss_func(A, C, G, H, U, V, W, outl1, outl2, outl3, alpha,
                             beta, gamma)
            early_stop(loss)
