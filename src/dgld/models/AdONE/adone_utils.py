...
def set_subargs(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of discriminator updates",
    )
    parser.add_argument("--gen_update_times",
                        type=int,
                        default=5,
                        help="number of generator updates")
    parser.add_argument("--lr_all",
                        type=float,
                        default=0.001,
                        help="learning rate")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="maximum length of the truncated random walk",
    )
    parser.add_argument("--restart",
                        type=float,
                        default=0.0,
                        help="probability of restart")
    parser.add_argument("--num_neighbors",
                        type=int,
                        default=-1,
                        help="number of sampling neighbors")
...
def random_walk_with_restart(g: dgl.DGLGraph, k=3, r=0.3, eps=1e-5):
    ...
    inv_degree = torch.where(torch.isinf(inv_degree),
                             torch.full_like(inv_degree, eps), inv_degree)
...
def loss_func(
    ...
    if pretrain is True:
        ...
        loss_hom_str = torch.mean(torch.log(torch.pow(os, -1)) *
                                  hom_str)  # Eq.3
        loss_prox_attr = torch.mean(torch.log(torch.pow(oa, -1)) * da)  # Eq.4
        loss_hom_attr = torch.mean(torch.log(torch.pow(oa, -1)) *
                                   hom_attr)  # Eq.5
        loss_com = 0.0

    if pretrain is False and train_disc is True:
        dc = (
            bce_wl(dis_a, torch.zeros_like(dis_a), reduction="none") +
            bce_wl(dis_s, torch.ones_like(dis_s), reduction="none")).squeeze()
        loss_disc = torch.mean(dc)
        return loss_disc

    dc = (bce_wl(dis_a, torch.ones_like(dis_a), reduction="none") +
          bce_wl(dis_s, torch.zeros_like(dis_s), reduction="none")).squeeze()
    ...
    for sg in dataloader:
        ...
        if pretrain is False:
            for i in range(disc_update_times):
                ...
                h_s, x_hat, h_a, c_hat, hom_str, hom_attr, dis_a, dis_s = model(
                    sg, sub_adj, feat)
                loss_disc = loss_func(
                    sub_adj,
                    x_hat,
                ...
                if verbose:
                    ...
                    print(
                        "disc attr:\t",
                        "mean={:.4f}".format(
                            torch.mean(torch.sigmoid(dis_a)).item()),
                        ", max={:.4f}".format(
                            torch.max(torch.sigmoid(dis_a)).item()),
                        ", min={:.4f}".format(
                            torch.min(torch.sigmoid(dis_a)).item()),
                    )
                    print(
                        "disc struct:\t",
                        "mean={:.4f}".format(
                            torch.mean(torch.sigmoid(dis_s)).item()),
                        ", max={:.4f}".format(
                            torch.max(torch.sigmoid(dis_s)).item()),
                        ", min={:.4f}".format(
                            torch.min(torch.sigmoid(dis_s)).item()),
                    )
