...
def set_subargs(parser):
    ...
    parser.add_argument("--n_enc_1",
                        type=int,
                        default=2000,
                        help="number of encode1 units")
    parser.add_argument("--n_enc_2",
                        type=int,
                        default=500,
                        help="number of encode2 units")
    parser.add_argument("--n_enc_3",
                        type=int,
                        default=128,
                        help="number of encode2 units")
...
def loss_func(B, B_hat, z_mean, z_arg, adj, A_hat, attrs, X_hat, alpha, eta,
              theta, device):
...
def train_step(model, optimizer, graph, features, B, adj_label, alpha, eta,
               theta, device):
    model.train()
    optimizer.zero_grad()
    A_hat, X_hat, B_hat, z, z_a = model(graph, features, B)
    # A_hat, X_hat = model(features,adj)
    loss, structure_cost, attribute_cost, kl_loss, re_loss, reconstruction_errors = (
        loss_func(
            B,
            B_hat,
            z,
    ...
    return (
        ...
        kl_loss,
        re_loss,
        reconstruction_errors.detach().cpu().numpy(),
    )
