...
from .comga_utils import test_step
from .comga_utils import train_step
...
class ComGA(nn.Module):
    ...
    def __init__(self, num_nodes, num_feats, n_enc_1, n_enc_2, n_enc_3,
                 dropout):
        super(ComGA, self).__init__()
        self.model = ComGAModel(
            num_nodes=num_nodes,
            num_feats=num_feats,
            n_enc_1=n_enc_1,
            n_enc_2=n_enc_2,
    ...
    def fit(
        ...
        early_stop = EarlyStopping(early_stopping_rounds=patience,
                                   patience=patience)
        ...
        for epoch in range(num_epoch):
            ...
            print(
                ...
                "{:.5f}".format(loss.item()),
                "train/kl_loss=",
                "{:.5f}".format(kl_loss.item()),
                "train/struct_loss=",
                "{:.5f}".format(struct_loss.item()),
                "train/feat_loss=",
    ...
    def predict(self, graph, alpha=0.7, eta=5.0, theta=40.0, device="cpu"):
        ...
        adj_label = adj_label.to(device)
        B = B.to(device)
        predict_score = test_step(self.model, graph, features, B, adj_label,
                                  alpha, eta, theta, device)
...
class ComGAModel(nn.Module):
    ...
    def __init__(
        ...
        self.commAE = CommunityAE(num_nodes, n_enc_1, n_enc_2, n_enc_3,
                                  dropout)
        self.tgcnEnc = tGCNEncoder(num_feats, n_enc_1, n_enc_2, n_enc_3,
                                   dropout)
        self.attrDec = AttrDecoder(num_feats, n_enc_1, n_enc_2, n_enc_3,
                                   dropout)
...
class CommunityAE(nn.Module):
    ...
    def __init__(self, num_nodes, n_enc_1, n_enc_2, n_enc_3, dropout):
...
class tGCNEncoder(nn.Module):
    ...
    def __init__(self, in_feats, n_enc_1, n_enc_2, n_enc_3, dropout):
